...
export default async function CssRoadmapPage() {
  ...
  const [tags, progress, newToday] = await Promise.all([
    prisma.tag.findMany({ where: { track: "css" }, orderBy: { order: "asc" } }),
    prisma.userTagProgress.findMany({ where: { userId, tag: { track: "css" } } }),
    prisma.userTagProgress.count({
      where: { userId, createdAt: { gte: startOfDay }, tag: { track: "css" } },
    }),
  ]);
  ...
  const gate = await getGate(userId, "css");

  // Giữ thứ tự lộ trình: Phần → Chương (topic) theo order của mục
    ...
    <div className="space-y-8 py-10">
      <div className="animate-rise">
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="font-display text-3xl font-bold tracking-tight">Lộ trình CSS</h1>
          <span className="rounded-full bg-sky-100 px-3 py-1 text-sm font-semibold text-sky-700">
            Beta
          </span>
        </div>
        <p className="mt-2 max-w-2xl leading-relaxed text-ink/60">
          CSS không học theo thẻ mà học theo <strong>phần</strong>: nền tảng trước, rồi giao diện,
          bố cục và hiệu ứng. Mỗi ngày 5 mục mới + ôn lại theo lịch, giống cách bạn học HTML.
        </p>
      ...
      <p className="text-sm text-ink/50">
        🚧 Beta: Phần 1 (Nền tảng) đã mở. Phần 2 Giao diện, Phần 3 Bố cục, Phần 4 Hiệu ứng đang
        được xây dựng và sẽ tự xuất hiện tại đây.
      </p>
    </div>
  );
}
