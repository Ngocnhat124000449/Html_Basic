...
export default async function ReactRoadmapPage() {
  ...
  const [tags, progress, newToday] = await Promise.all([
    ...
    prisma.userTagProgress.count({
      where: { userId, createdAt: { gte: startOfDay }, tag: { track: "react" } },
    }),
  ]);
  ...
  const gate = await getGate(userId, "react");
      ...
      <div className="animate-rise">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          {gate ? (
            ...
            <Link
              href="/study?track=react&mode=learn"
            ...
            <div className="rounded-2xl border border-ink/10 bg-surface p-6">
              ...
              {counts.unseen > 0 ? (
                <>
                  ...
                  <Link
                    href="/study?track=react&mode=learn&extra=1"
          ...
          {counts.due > 0 ? (
            <Link
              href="/study?track=react&mode=review"
