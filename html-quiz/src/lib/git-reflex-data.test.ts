...
import { GIT_REFLEX_QUESTIONS } from "./git-reflex-data";

// Cùng cách chuẩn hóa với reflex-game để bảo đảm đáp án chấm được.
const norm = (s: string) => s.toLowerCase().split("=")[0].replace(/[<>"'`/\s*]/g, "");

describe("Phản xạ Git", () => {
  it("có đủ câu hỏi", () => {
    expect(GIT_REFLEX_QUESTIONS.length).toBeGreaterThanOrEqual(30);
  });

  it("mỗi câu có answer, prompt, explain hợp lệ", () => {
    for (const q of GIT_REFLEX_QUESTIONS) {
  ...
  it("không trùng answer", () => {
    const a = GIT_REFLEX_QUESTIONS.map((q) => q.answer);
